...
const handlePostMoveNotification = async ({ game, mover, opponent }) => {
    ...
    if (isGameOver) {
        let winnerMessage = `You beat ${mover.username} in a game of tic tac toe!`;
        let loserMessage = `Ahh, you lost to ${opponent.username} in tic tac toe.`;

        await Promise.all([
            sendMessage2({ senderEmailAddress: mover.email, receiverEmailAddress: opponent.email, message: winnerMessage }),
            sendMessage2({ senderEmailAddress: opponent.email, receiverEmailAddress: mover.email, message: loserMessage })
        ]);
    } else {
    ...
    return { isGameOver, winner, winnerMessage, loserMessage };
};
